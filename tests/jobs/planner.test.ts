...
describe("planJob", () => {
  ...
  it("charges one candle call per buy so the planned figure bounds what the step actually spends", async () => {
    ...
    expect(plan.plannedRequests).toBe(5 + 5 * 2 + 3);
  });

  it("reserves two calls per wallet outside the discovery budget, so every top wallet is fetched", async () => {
    const db = openTestDb();
  ...
  it("stops discovery when the step has run out of wall clock and still returns a usable plan", async () => {
    ...
    expect(client.requests).toBe(2 + 3 + 2);
