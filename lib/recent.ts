...
export async function handleRecent(ctx: AppContext, chain: string, addr: string, ip: string): Promise<Response> {
  ...
  try {
    ...
    const fallback = newestOf(await loadBuys(ctx.db, chain, wallet, MAX_BUYS_PER_WALLET));
    if (!fallback) return Response.json({ newest: null, buyers: [], status: "none", stale: true, reason }, { status: 503 });
    return Response.json({ newest: fallback, buyers: [], status: "provisional", stale: true, reason });
  }
}
